// Same methodology as analyze.mjs (identical KEYWORDS regexes, identical JD
// corpus) but run against the resume's plain text instead of the site's, so
// resume vs. site coverage is directly, apples-to-apples comparable.
...
const resumeText = fs.readFileSync("out/resume-source.txt", "utf8");
...
for (const [label] of KEYWORDS) {
  ...
  const onResume = KEYWORDS.find(k => k[0] === label)[1].some(r => r.test(resumeText));
  if (jdHits.length > 0 && !onResume) {
    gaps.push({ keyword: label, postingCount: jdHits.length, sources: jdHits.map(h => ({ company: h.company, title: h.title, url: h.url })) });
  } else if (jdHits.length > 0 && onResume) {
...
fs.writeFileSync("out/resume-keyword-coverage.json", JSON.stringify(output, null, 2));
console.error(`Resume analyzed against ${postings.length} postings. Gaps: ${gaps.length}, Covered: ${covered.length}`);
